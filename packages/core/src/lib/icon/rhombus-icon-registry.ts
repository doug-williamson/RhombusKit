...
const RHOMBUS_DEFAULT_GLYPHS: Record<string, RhombusGlyph> = {
  ...
  // Stepper step-state indicators (Material "filled"). The `number` state renders
  // the step index as text, so only edit/done/error need a glyph.
  edit: {
    viewBox: '0 0 24 24',
  ...
  // Reorder list functional affordances (Material "filled"). drag_indicator is
  // the grab handle; the arrows label the move-up / move-down buttons.
  drag_indicator: {
    viewBox: '0 0 24 24',
