...
| indicator glyph (`:105-107`) | `rhombus-icon { font-size: 1.125rem; width: 1.125rem; height: 1.125rem }` | **change to `em`** — the one Tier 2 icon that must be addressed. If the 28px bubble shrinks at compact and the 18px glyph does not, it overflows. The bubble sets `font-size: 0.8125rem` (`:98`), so `1.125rem ≈ 1.385em`. **Local fix in this file; the icon primitive is not touched** (§7.14). |
...
- **stepper indicator** — `rhombus-icon` descendant selector at `:104-107` → convert to `em` (§7.12).
...
|---|---|---|---|
| **D1** | **The stepper glyph `em` conversion** (§7.12) is a rendered-geometry change inside PR 3, however small. Ship it inside the density PR or split it? | **Ship it in PR 3**, flagged in the geometry-baseline diff and in the changelog. It is 3 lines in one file, it is a prerequisite for compact rendering correctly, and holding it back means shipping a known-broken compact stepper. | Split it into its own patch first. Cleaner baseline story, one more PR. |
