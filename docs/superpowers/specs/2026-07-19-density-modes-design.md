...
| 7 | `apps/showcase/src/app/pages/roadmap/roadmap-data.ts` | Move the sole `foundations.next` item (`:124-130`) into `foundations.now` with a release tag. **Refill `next`** by promoting two genuinely-unshipped `considering` entries and deleting them from `considering`; without the refill the column renders empty. **Correction (PR 3):** the "Theme registry" `considering` entry is **stale — that epic already shipped (#87–#92)** and must be dropped from the roadmap, not promoted. Promote **High-contrast theme** and **Theme builder** instead. |
